  ...
  return (
    <header className="sticky top-0 z-40 h-[68px] flex items-center bg-white/50 backdrop-blur-2xl border-b border-[#ECE8DF] px-4 md:px-8 gap-4">
      <button
        ...
        className="md:hidden w-9 h-9 flex items-center justify-center rounded-xl text-[#6B6F78] hover:bg-white transition-colors"
      >
        {mobileOpen ? <X className="w-[18px] h-[18px]" strokeWidth={1.85} /> : <Menu className="w-[18px] h-[18px]" strokeWidth={1.85} />}
      </button>

      <h1 className="text-[19px] font-serif font-semibold text-[#17181C] tracking-tight flex-1">
        {pageTitle}
      </h1>
        ...
        {mobileOpen && (
          <>
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="fixed inset-0 top-[68px] bg-[#17181C]/25 backdrop-blur-sm z-30"
            ...
            <motion.nav
              ...
              className="fixed left-0 top-[68px] bottom-0 w-72 bg-white/90 backdrop-blur-2xl border-r border-[#ECE8DF] z-40 p-4 space-y-1 overflow-y-auto"
            >
              {navItems.map((item) => {
