...
export default function TicketDetailPage() {
  ...
  if (notFound) {
    return (
      <div className="max-w-2xl mx-auto space-y-6">
        <Link href="/settings/reports" className="inline-flex items-center gap-1.5 text-[13px] text-[#8A8E96] hover:text-[#3A3D45]">
          <ArrowLeft className="w-3.5 h-3.5" />
          Back to My Reports
        </Link>
        <Panel className="p-8 text-center">
          <p className="text-[14px] text-[#8A8E96]">This report doesn't exist or you don't have access to it.</p>
        </Panel>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        <Link href="/settings/reports" className="inline-flex items-center gap-1.5 text-[13px] text-[#8A8E96] hover:text-[#3A3D45] mb-3">
          <ArrowLeft className="w-3.5 h-3.5" />
          Back to My Reports
        </Link>
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <p className="text-[12.5px] text-[#8A8E96] mt-0.5">
                {CATEGORY_LABELS[ticket.category] || ticket.category} · Opened {formatDate(ticket.createdAt)}
              </p>
            </div>
            <span className={`shrink-0 text-[11.5px] font-medium px-2.5 py-1 rounded-full ${STATUS_STYLES[ticket.status] || ''}`}>
          ...
          <div key={r.id} className={`flex ${r.isAdmin ? 'justify-start' : 'justify-end'}`}>
            <div
              className={`max-w-[80%] rounded-[16px] px-4 py-3 ${
                r.isAdmin ? 'bg-white border border-[#ECE8DF]' : 'bg-[#FF7A45]/10 border border-[#FF7A45]/20'
              }`}
            >
              <p className="text-[13px] text-[#17181C] whitespace-pre-wrap">{r.message}</p>
              <p className="text-[11px] text-[#B0B4BB] mt-1">
                {r.isAdmin ? 'Support' : 'You'} · {formatDate(r.createdAt)} {formatTime(r.createdAt)}
              </p>
            </div>
          </div>
        ))}
